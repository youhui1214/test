...
```javascript
 // 1, Set：独一无二的集合，数组，对象，字符串去重，有遍历器接口，可遍历for... of
 // 2, WeakSet: 只能对象，没有遍历器接口，不能引用
 
 // 1, Map: 键值对集合，但key不只是字符串，可以是各种值，包括对象，for...of
 // 2, WeakMap: 键值对集合，只接受对象，没有遍历器接口
